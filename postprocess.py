    ...
    with h5py.File(h5file_name, "r") as h5file:
        ...
        for delta_x in sigma_vec:
            ...
            for i, lab in enumerate(labels):
                # ax_mismatch[i].semilogy(time[5:]/86400, mismatch_upp[i][5:], label=f"{int(delta_x)}-sigma", alpha=0.8)
        ...
        for delta_x in sigma_vec:
            ...
            plt.colorbar(scatter, label="log10 Mismatch")
            ...
            plt.grid(alpha=0.3)
            plt.savefig(os.path.join(results_dir, f"sky_location_mismatch_sigma_{int(delta_x)}_channel{ind_channel}_{h5file_name.split('.h5')[0]}.png"))
            plt.close()
